...
### Added
- **Language Features**: Added support for Classes (OOP), including inheritance (`extends`), constructors, and methods.

### Fixed
- **Installer**: Fixed icon path in `setup.iss` that caused compilation errors.
- **Build Scripts**: Fixed PowerShell compatibility issues in `package.json` scripts (replaced `&&` with `;`).

## [1.0.1] - 2026-02-08

### Fixed
- **Console Error**: Fixed "mel-setup tag not found" error spam in the browser console by making the check silent.
- **Installer**: Updated installer to version 1.0.1.
