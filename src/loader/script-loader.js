...
(function() {
    ...
    console.error = function(...args) {
        ...
        if (melScript && melScript.ERROR_TYPES && typeof melScript.ERROR_TYPES === 'object' && melScript.ERROR_TYPES.params) {
            return;
        }
        ...
        if (melScript && melScript.CONFIG === "web") {
            ...
            if (errorMsg.includes('Error in:') || errorMsg.includes('LINE:')) {
                ...
                errorWindow.style.cssText = 'position:fixed;top:20px;left:50%;transform:translateX(-50%);width:90%;max-width:600px;background:#fff;border:1px solid #0078d4;border-radius:8px 8px 0 0;box-shadow:0 4px 20px rgba(0,0,0,0.3);z-index:99999;font-family:Segoe UI,Arial,sans-serif;';
                
                const titleBar = document.createElement('div');
                titleBar.style.cssText = 'background:linear-gradient(to bottom, #fff 0%, #f0f0f0 100%);border-bottom:1px solid #d0d0d0;padding:8px 10px;display:flex;align-items:center;justify-content:space-between;cursor:move;user-select:none;border-radius:8px 8px 0 0;';
                
                const titleLeft = document.createElement('div');
                titleLeft.style.cssText = 'display:flex;align-items:center;gap:8px;';
                
                const icon = document.createElement('div');
                icon.style.cssText = 'width:16px;height:16px;background:#ff0000;border-radius:2px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:12px;font-weight:bold;';
                icon.textContent = '✕';
                ...
                maximizeBtn.style.cssText = 'width:45px;height:29px;background:transparent;border:none;color:#000;font-size:16px;cursor:pointer;transition:background 0.2s;';
                maximizeBtn.onmouseover = function() { this.style.background = '#e0e0e0'; };
                maximizeBtn.onmouseout = function() { this.style.background = 'transparent'; };
                ...
                closeBtn.innerHTML = '✕';
                closeBtn.style.cssText = 'width:45px;height:29px;background:transparent;border:none;color:#000;font-size:16px;cursor:pointer;transition:background 0.2s;';
                closeBtn.onmouseover = function() { this.style.background = '#e81123'; this.style.color = '#fff'; };
                closeBtn.onmouseout = function() { this.style.background = 'transparent'; this.style.color = '#000'; };
                closeBtn.onclick = function() { errorWindow.remove(); };
                ...
                errorContent.style.cssText = 'margin:0;font-family:Consolas,monospace;font-size:13px;color:#000;white-space:pre-wrap;word-wrap:break-word;';
