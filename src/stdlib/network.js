...
function createSmartCallable(fn) {
  ...
  if (mode === 'new') {
    const wrapper = function(...args) { return new fn(...args); };
...
Lang.addHandler('js', {
  ...
  call: (target, args, scope) => {
    ...
    if (hasExport) {
      ...
      const libName = url.split('/').reverse().find(p => p && p !== 'dist' && p !== 'build' && !p.includes('.'))
                         ?.replace(/-es$/, '').replace(/-/g, '_').toUpperCase() || 'LIB';
